...
let rec max_list (lst : int list) : int =
  match lst with
    [a] -> a
  | [] -> raise (Invalid_argument "bad argument")
...
let rec zip (x : int list) (y : int list) : (int * int) list =
  match x, y with
    [], [] -> []
  | [], _ -> raise (Invalid_argument "lists different lengths" )
  | _, [] -> raise (Invalid_argument "lists different lengths" )
...
let evens  (lst :int list) : int list =
  List.fold_left (fun x y -> if y mod 2 = 0 then ([y] @ x) else (x)) [] lst;;
